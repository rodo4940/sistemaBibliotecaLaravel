...
<section class="p-6 bg-gray-100 min-h-full overflow-auto">
  <div class="max-w-lg mx-auto bg-white border border-gray-200 rounded-lg shadow p-6 space-y-6">
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-semibold text-gray-800">Agregar categoría</h1>
      <a href="{{ route('categories') }}" class="text-indigo-600 hover:underline">Todas las categorías</a>
    </div>

    <form action="{{ route('category.store') }}" method="POST" autocomplete="off" class="space-y-4">
      @csrf
      <div>
        <label for="name" class="block text-sm font-medium text-gray-700">Nombre de la categoría</label>
        <input id="name" name="name" type="text" required placeholder="Nombre de la categoría"
