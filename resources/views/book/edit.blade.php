...
<section class="p-6 bg-gray-100 min-h-full overflow-auto">
  <div class="max-w-lg mx-auto bg-white border border-gray-200 rounded-lg shadow p-6 space-y-6">
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-semibold text-gray-800">Actualizar libro</h1>
      <a href="{{ route('books') }}" class="text-indigo-600 hover:underline">Todos los libros</a>
    </div>

    <form action="{{ route('book.update', $book->id) }}" method="POST" autocomplete="off" class="space-y-4">
      @csrf
      @method('PATCH')
      ...
      <div>
        ...
        <select id="auther_id" name="auther_id" required
          ...
          @foreach($authors as $author)
            <option value="{{ $author->id }}" {{ old('auther_id', $book->auther_id) == $author->id ? 'selected' : '' }}>{{ $author->name }}</option>
          @endforeach
        </select>
